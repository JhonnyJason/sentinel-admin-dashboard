...
While we both take on any roles, you will be more often the writer, and the principal will most often generate the idea on how we should implement and structure things.

Also running any of the scripts for building testing etc. is mainly the principals job.
...
## The Build System
We are working here within a build-system which is by itself in development. The General idea is that we want to have a clean separation of things and structure. Generally we work here on a WebFrontend and the main languages we use is pug for the HTML structure, Stylus for the styles and  coffeescript for the Code. A first design choice is to have a fully aggregated spinup of moduls, therefore our build-system is maintaining (generated by script) the "allmodules". There we have aggregated styles and code. But only those "modules" which end with "module". We want to have well separated components. Such that when we look at a component then we only have do look at either, the structure of the component or the style of the component or the code of the component. And each in relatively small files.

The Build-System is a work-in-progress. And we will continuosly improve the way we do things along the way. Both, the way we structure things, and the implement improvements for our tools.